...
for id, tiletype in data.tiletypes.items():
    ...
    if src_file.is_file():
        ...
        subprocess.check_call(['convert', '-resize', f'{png_size}x{png_size}', '-background', bgcolor, dst_file, png32_file])
    else:
        # TODO: red-fill explicitly invalidated tile types
        generate_blank_image(png32_file, png_size, png_size, bgcolor)
...
<tileset name="biomes" tilewidth="{png_size}" tileheight="{png_size}" tilecount="{len(tiletype_list)}" columns="0">
...
''')
    ...
    for i, id in enumerate(data.biomes.keys()):
        ...
        bgcolor = data.bgpalette[biome['bgcolor']]

        png = biomes_png_path / (str(id) + '.png')
        generate_blank_image(png, png_size, png_size, bgcolor)

        tsx.write(f'''\
  <tile id="{i}" type="{biome['name']}">
    <image width="{png_size}" height="{png_size}" source="{png}"/>
  </tile>
''')
...
with open(studio_data_path / 'tileset.tsx', 'wt') as tsx:
    tsx.write(f'''\
<?xml version="1.0" encoding="UTF-8"?>
<tileset name="terrain_tiles" tilewidth="{png_size}" tileheight="{png_size}" tilecount="{len(tiletype_list)}" columns="0">
  <grid orientation="orthogonal" width="1" height="1"/>
''')
  ...
  <tile id="{i}" type="{tiletype['name']}">
    <image width="{png_size}" height="{png_size}" source="{png32_file}"/>
  </tile>
''')
